...
import type { ISubinstructionPayloadUpdate } from "~/types/Subinstruction/type";
import type { ISubinstruction } from "~/types/Subinstruction/ISubinstruction";

const props = defineProps<{
  dialog: boolean;
  subinsruction: ISubinstruction;
}>();
...
const data = ref<ISubinstructionPayloadUpdate>(props.subinsruction);
...
const updateSubinstruction = () => {
  Subinstruction.update(data.value)
    .then(() => {
      toast.success("Подинструкция измененна");
      ...
      >
        <v-card-title>
          <div class="flex justify-between">
            <div class="text-h5"> Редактирование подинструкции </div>
