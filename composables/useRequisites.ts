...
export class Requisites {
  ...
  static readonly NALOG_STATUS_DESCRIPTION: { [key: number]: string } = {
    [this.PHYSICAL]:
      "Обратите внимание, что при выборе статуса «Физическое лицо», из вашего вознаграждения за аренду будет удерживаться НДФЛ в размере 13% от размера вознаграждения.",
    [this.SELF_EMPLOYED]:
      'Обратите внимание, что при выборе статуса «Самозанятый», после каждой выплаты за аренду квартиры необходимо прикреплять чеки в разделе <span class="font-bold">"Документы"</span>',
  };
